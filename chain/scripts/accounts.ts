import { privateKeyToAccount } from "viem/accounts";

export const LOCAL_KEYS = {
  deployer: "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80",
  challenger: "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d",
  opponent: "0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a",
  facilitator: "0x7c852118294e51e653712a81e05800f419141751be58f605c371e15141b007a6",
  treasury: "0x47e179ec1974885a0f2f6fe3e43d1b860d94d7f4c6a6dbb688a96b8387020a9d",
} as const;

export const LOCAL_ACCOUNTS = {
  deployer: privateKeyToAccount(LOCAL_KEYS.deployer),
  challenger: privateKeyToAccount(LOCAL_KEYS.challenger),
  opponent: privateKeyToAccount(LOCAL_KEYS.opponent),
  facilitator: privateKeyToAccount(LOCAL_KEYS.facilitator),
  treasury: privateKeyToAccount(LOCAL_KEYS.treasury),
} as const;
