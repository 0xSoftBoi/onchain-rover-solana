...
const debugPort = Number(process.env.CHROME_DEBUG_PORT ?? 9304);
...
const pilotUrl = process.env.PILOT_URL
  ?? `${sidecarBase}/pilot-react.html?robot=${encodeURIComponent(robot)}&mode=manual&transport=webrtc&speed=medium`;
...
async function main() {
  ...
  try {
    ...
    await waitForExpression(cdp, sessionId, "window.__pilotDebug?.dataChannelOpen === true", 10000);
    await waitForExpression(cdp, sessionId, "document.querySelector('#startModal')?.classList.contains('hidden') === true");

    const points = await evaluate<{ center: { x: number; y: number }; target: { x: number; y: number } }>(cdp, sessionId, `(() => {
      const el = document.querySelector('#driveZone');
      if (!el) throw new Error('missing drive zone');
      const r = el.getBoundingClientRect();
      const center = { x: r.left + r.width / 2, y: r.top + r.height / 2 };
      const target = {
        x: center.x + Math.min(62, r.width * 0.32),
        y: center.y - Math.min(70, r.height * 0.36),
      };
...
async function waitForExpression(client: CdpClient, sessionId: string, expression: string, timeoutMs = 6000) {
...
async function readRobotState(client: CdpClient, sessionId: string): Promise<any> {
  return evaluate(client, sessionId, `fetch('/robot-link/state').then((res) => res.json()).then((state) => state.robots[${JSON.stringify(robot)}])`);
}
...
function commandSummary(state: any) {
  return {
    ...
    webrtcPilotClients: state?.webrtcPilotClients,
