...
- [x] **Live cash-out** mid-race (odds-driven sell-back before settle) — marquee sportsbook feature. Built: sidecar `/race/cashout` quote (`value = stake × entryOdds/currentOdds × (1−5% margin)`, capped at full payout) + a LIVE CASH-OUT card on the betting panel that auto-arms a $5 demo position on the favourite and ticks the value every 1.3s as odds move (green profit / red loss), with a CASH OUT button that locks it via a moment + screen-shake and re-arms in 5s. Server-authoritative quote with a client-side fallback for pure-mock standalone. Verified directionality: flat→−margin, side shortens→profit, side drifts→loss; verify.sh --sidecar OK.
